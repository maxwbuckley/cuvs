                            ...
                            uint32_t universe_size)
{
  ...
  raft::update_device(
    result.types.data(),
    reinterpret_cast<const roaring_container_type*>(h_types.data()), n, stream);
                          ...
                          uint32_t output_size_words)
{
  ...
  decompress_kernel<<<bitmap.n_containers, 256, 0, stream>>>(
    bitmap.keys.data(),
    bitmap.types.data(),
    bitmap.offsets.data(),
...
static HostIndex download_index(const gpu_roaring& g, cudaStream_t stream)
{
  ...
  RAFT_CUDA_TRY(cudaMemcpyAsync(
    h.types.data(), g.types.data(), n * sizeof(roaring_container_type), cudaMemcpyDeviceToHost, stream));
  RAFT_CUDA_TRY(cudaMemcpyAsync(
                      ...
                      cudaStream_t stream)
{
  switch (op) {
    ...
    case roaring_set_op::ANDNOT:
      bitmap_bitmap_kernel<roaring_set_op::ANDNOT><<<n, 256, 0, stream>>>(
        d_work, n, a, b, out, cards);
      break;
    case roaring_set_op::XOR:
      bitmap_bitmap_kernel<roaring_set_op::XOR><<<n, 256, 0, stream>>>(
        d_work, n, a, b, out, cards);
      break;
  }
}
                   ...
                   roaring_set_op op)
{
  ...
  while (ia < a.n_containers && ib < b.n_containers) {
    if (ha.keys[ia] < hb.keys[ib]) {
      ...
      if (op == roaring_set_op::OR || op == roaring_set_op::XOR)
        work.push_back(
          {hb.keys[ib], -1, (int32_t)ib, roaring_container_type::ARRAY, hb.types[ib]});
      ++ib;
    } else {
  ...
  uint32_t out_n_bitmap =
    static_cast<uint32_t>(bb_work.size() + mixed.size() + copy_bmp.size());

  // Allocate output bitmap pool
  rmm::device_uvector<uint64_t> d_out_bmp(
    static_cast<size_t>(out_n_bitmap) * 1024, stream);
  ...
  if (!bb_work.empty()) {
    ...
    raft::update_device(d_bb.data(), reinterpret_cast<const BitmapBitmapWork*>(bb_work.data()),
                        n_bb, stream);

    launch_bb(op, n_bb, d_bb.data(), a.bitmap_data.data(), b.bitmap_data.data(),
              d_out_bmp.data(), d_bb_cards.data(), stream);
    RAFT_CUDA_TRY(cudaPeekAtLastError());
  ...
  if (!mixed.empty()) {
    ...
    launch_bb(op, n_mx, d_mx_bb.data(), d_tmp_a.data(), d_tmp_b.data(),
              d_out_bmp.data() + static_cast<size_t>(bb_sz) * 1024,
              d_mx_cards.data(), stream);
    RAFT_CUDA_TRY(cudaPeekAtLastError());
  ...
  for (auto& wi : work) {
    if (wi.a_idx < 0 || wi.b_idx < 0) {
      ...
      if (ta == roaring_container_type::BITMAP && tb == roaring_container_type::BITMAP) {
        ...
        if (card == 0 && op != roaring_set_op::OR) { ++bb_idx; continue; }
        out_keys.push_back(wi.key);
        out_types.push_back(roaring_container_type::BITMAP);
        ...
        if (card == 0 && op != roaring_set_op::OR) { ++mx_idx; continue; }
  ...
  if (result.n_containers > 0) {
    ...
    raft::update_device(
      result.cardinalities.data(), out_cards.data(), result.n_containers, stream);
    raft::resource::sync_stream(res);
  }
